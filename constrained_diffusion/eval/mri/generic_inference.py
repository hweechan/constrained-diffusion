...
from constrained_diffusion.eval.mri.dataset import load_dataset
from constrained_diffusion.eval.mri.model import load_model
...
def main(
    ...
    dataset_name="HumanEval/FIM/cpp/1",
    device="cuda",
    temp=0,
    seed=0,
    max_tokens=128,
    ...
    for instance in tqdm(sorted(dataset, key=lambda x: x.instance_id())[:limit]):
        ...
        if not constrained:
            prompt, code, extracted, timed_out, generated_tokens = (
                eval_model.generate_unconstrained(
                    instance,
                    model,
                    tokenizer,
